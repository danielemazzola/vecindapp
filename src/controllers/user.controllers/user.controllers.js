  ...
  try {
    ...
    if (!geoData?.length) {
      return res.status(400).json({
        message: 'Unable to fetch geolocation data. Please verify the address and try again.'
      });
    }
    ...
    if (error.name === 'ValidationError') {
      return res.status(400).json({ message: 'Invalid user data. Please verify the provided information.' });
    }
