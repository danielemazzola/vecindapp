...
const PURCHASE = async (req, res, next) => {
  try {
    ...
    if (!user || !license) {
      return res
        .status(400)
        .json({ message: 'Missing user or license information.' })
    }

    if (!license.isActive) {
      return res
        .status(401)
        .json({ message: 'The selected license is inactive.' })
    }
    ...
    if (!savedAssignment) {
      return res.status(500).json({
        message:
          'The system failed to create a license assignment. Please try again.',
      })
    }
